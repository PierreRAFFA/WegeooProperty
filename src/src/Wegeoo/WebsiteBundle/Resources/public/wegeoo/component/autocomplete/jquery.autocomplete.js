...
(function($) {
    ...
    $.Autocompleter = function($elem, options) {
        ...
        var defaultCityName = $elem.attr("data-cityname");
        var defaultCityPostCode = $elem.attr("data-citypostcode");

        if( defaultCityName && defaultCityPostCode)
        {
            var lSelectedCity = {};
            lSelectedCity.postCode   = defaultCityPostCode;
            lSelectedCity.libelle       = defaultCityName;
            lSelectedCity.uppercaseName = defaultCityName.toUpperCase();

            //remove these attributes to prevent to reload them after.
            $elem.removeAttr("data-cityname");
            $elem.removeAttr("data-citypostcode");
    ...
    $.Autocompleter.prototype.position = function() {
        ...
        if ( this.options.maxHeight)
       		vMaxHeight = this.options.maxHeight;
		this.dom.$results.css({
			top: offset.top + this.dom.$elem.height()+3+2,
			left: offset.left,
			'max-height':'300px',
			'z-index':500,
			'overflow-y':"visible",
	...
	$.Autocompleter.prototype.filterResults = function(results, filter) {
		...
		if (this.options.useFilter == false) {
			return results;
		}else{
	...
	$.Autocompleter.prototype.showResults = function(results, filter) {
		...
		extraWidth = 0;
		this.dom.$results.width(this.dom.$elem.width() - extraWidth);
	...
	$.Autocompleter.prototype.selectItem = function($li)
	{
		...
		this.selectItemFromDataAndValue(value,data);
	};

    $.Autocompleter.prototype.selectItemFromDataAndValue = function(pValue,pData)
    {
        var displayValue = this.displayValue(pValue, pData);
        ...
        if(typeof options === 'string') 
        {
        	if ( options === "destroy")
        	{
        		if ( $(this).data('autocompleter') )
        			$(this).data('autocompleter').remove();
    ...
    $.fn.getSelectedCityPostCode = function ()
    {
        var lPostCode = null;
        if ( $(this).data('autocompleter') )
        {
    ...
    $.fn.setSelectedCityPostCode = function (postCode)
    {
        if ( $(this).data('autocompleter') )
        {
            $(this).data('autocompleter').setSelectedCityPostCode(postCode);
        }
    };
    $.fn.setSelectedCityName = function (cityName)
    {
        if ( $(this).data('autocompleter') )
        {
            $(this).data('autocompleter').setSelectedCityName(cityName);
        }
    };

    $.fn.getSelectedCityName = function ()
    {
        var lCityName = null;
        if ( $(this).data('autocompleter') )
        {
	...
	$.fn.setSelectedCity = function(pTownName, pTownPostalCode)
	{
	    ...
	    if ( $(this).data('autocompleter') )
	    {
	        var lTownValue = pTownName.toUpperCase();
	        
	        var lTownData = {};
	        lTownData.name          = pTownName;
	        lTownData.uppercaseName = pTownName.toUpperCase();
	        lTownData.postCode     = pTownPostalCode;
	        
	        $(this).data('autocompleter').selectItemFromDataAndValue(pTownName.toUpperCase(),lTownData );
	    }
	};
    /**
